...
(function($) {
	...
	Scene_Boot = class extends Scene_Boot {
		...
		start() {
			super.start();
			const isDesktopRuntime = Utils.isNwjs();
			this.determineOS();
			if(isDesktopRuntime && !!this.hasSteamwork()) {
				this.getAchievementsData();
			}
			else if(isDesktopRuntime && !this.hasSteamwork() && !Utils.isOptionValid("test")) {
			...
			if(isDesktopRuntime && Utils.isMac()) {
				const nw = require("nw.gui");
				const nw_window = nw.Window.get();
		...
		determineOS() {
			if(!Utils.isNwjs()) {
				Utils.setMac(/Mac|iPhone|iPad|iPod/i.test(navigator.platform || navigator.userAgent || ""));
				return;
			}
			const os = require("os");
...
(function($) {
	...
	Permanent_Manager = class {
		...
		static load() {
			...
			this._cutsceneData = {};
			const filepath = pp.join(folder, "CUTSCENE.json");
			fs.readFile(filepath, "utf8", (err, data) => {
				...
				try {
					...
					console.warn("CUTSCENE.json is corrupted. Resetting permanent cutscene data.");
					this._cutsceneData = {};
				}
			})
		}
	...
	LanguageManager = class extends LanguageManager {
		...
		static loadLanguageFiles(language) {
			...
			for(let directory of dirList) {
				...
				if(format === ".HERO" || format === ".yaml" || format === ".yml") {
	...
	AtlasManager = class extends AtlasManager {
		...
		static initAtlasData() {
			...
			if(window["$atlasData"] === undefined) {
				...
				var data = yaml.safeLoad(yy.toString());
